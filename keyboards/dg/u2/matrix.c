...
static bool custom_read_cols_on_row(matrix_row_t current_matrix[], uint8_t current_row) {
  ...
  for (uint8_t col_index = 0; col_index < MATRIX_COLS; col_index++) {
    bool isR2C = (current_row < LOGICAL_ROW_COUNT && col_index >= LOGICAL_ROW_COUNT);
    bool isC2R = (current_row >= LOGICAL_ROW_COUNT && col_index < LOGICAL_ROW_COUNT);

    // skip reading R2R or C2C
    if (isR2C || isC2R) {
